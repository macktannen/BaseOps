...
interface FileWithResize extends File {
  _resizeFailed?: boolean;
}
...
function resizeImage(file: FileWithResize): Promise<File> {
...
async function prepareFileForUpload(file: File): Promise<{ file: File; resizeFailed: boolean }> {
  if (!isImageFile(file)) return { file, resizeFailed: false };
  try {
    const result = await resizeImage(file as FileWithResize);
    return { file: result, resizeFailed: !!(result as FileWithResize)._resizeFailed };
  } catch (err) {
...
export const FileStorageService = {
  async saveFile(flightId: string, file: File): Promise<FileUploadResult> {
  ...
  async getFiles(flightId: string): Promise<FileListResult[]> {
    if (!storage) return [];
  ...
  async downloadFile(storagePath: string, fileObj: { name?: string } | null = null, defaultName: string = 'download'): Promise<void> {
    ...
    const url = await this.getFileUrl(storagePath);
    if (url) {
      const a = document.createElement('a');
      a.href = url;
      a.download = fileName;
  ...
  async saveReceipt(flightId: string, expenseId: string, file: File): Promise<FileUploadResult> {
  ...
  async deleteReceipt(storagePath: string): Promise<boolean> {
    if (storage && storagePath) {
      try {
        const fileRef = ref(storage, storagePath);
        await deleteObject(fileRef);
  ...
  async getReceipts(flightId: string, expenseId: string): Promise<FileListResult[]> {
    if (!storage) return [];
