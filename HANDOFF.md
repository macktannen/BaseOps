...
1. **Synchronous Direct Schedule Mutation**: Hardened `handleCellClick` in `CrewSchedule.jsx` to update React state `setSchedules({ ...stored })` synchronously upon clicking Clear, eliminating render lag or dropped state updates.
